...
import os, sys
import json
from bitshopapp.models import Product, ProductImage, Category, SubCategory, Brand
		...
		for item in data:															
			for key in item:
				if key=='category':
					...
					offerprice_temp2 = offerprice_temp.replace("[u'","").replace("']","").replace('[u"','').replace('"]','').replace(" ', u'","")
					if offerprice_temp2 == "[]":
						product_offerprice = ""
					else:
					...
					saleprice_temp2 = saleprice_temp.replace("[u'","").replace("']","").replace('[u"','').replace('"]','').replace(" ', u'","")
					if saleprice_temp2 == "[]":
						product_saleprice = ""
					else:
